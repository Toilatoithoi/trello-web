...
function Notifications() {
  ...
  const handleClickNotificationIcon = (event) => {
    ...
    // Khi click vào phần icon thông báo thì set lại trạng thái newNotification về false
  ...
  useEffect(() => {
    ...
    const onReceiveNewInvitation = (invitation) => {
      ...
      if (invitation.inviteeId === currentUser._id) {
        ...
        // Bước 2: Cập nhạt trạng thái đang có thông báo đến
        setNewNotification(true)
      }
    }

    // Lắng nghe một sự kiện real-time có tên là FE_USER_INVITED_TO_BOARD từ phía server gửi về
    socketIoInstance.on('FE_USER_INVITED_TO_BOARD', onReceiveNewInvitation)

    // Clean up event để ngăn chặn việc bị đăng ký lặp lại event (link docs ở trên)
    return () => {
