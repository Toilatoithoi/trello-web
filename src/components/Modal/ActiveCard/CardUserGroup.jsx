...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  */
  ...
  // Đoạn này lấy activeBoard từ redux ra để mục đích lấy được toàn bộ thông tin những thành viên của cái board thông qua FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)
  /**
    *Thành viên trong card sẽ phải là tập con của thành viên trong board
    *Vì thể dựa vào màng board. FE_allUsers và card.memberIds rồi chúng ta tạo ra một màng FE_CardMembers chứa đủ thông tin của User đề hiển thị ra ngoài giao diện, bởi mặc định trong card chỉ lưu đám Id của User thôi (memberIds)
  */

  // const FE_CardMembers = board?.FE_allUsers?.filter(user => cardMemberIds.includes(user._id))
  // Sắp xếp FE_CardMembers theo thứ tự của cardMemberIds khi thêm vào mảng FE_CardMembers
  const FE_CardMembers = cardMemberIds?.map(id => board.FE_allUsers.find(u => u._id === id))

  const handleUpdateCardMembers = (user) => {
    // Tạo một biến incomingMemberInfo để gửi cho BE, với 2 thông tin chính là userId và action là xoá khỏi card (REMOVE) hoặc thêm vào card (ADD)
