...
function AccountTab() {
  ...
  // Những thông tin của user đề init vào form (key tương ứng với register phía dưới Field)
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    toast.promise(
      ...
      // Đoạn này phải kiểm tra không có lỗi (update thành công) mới thực hiện các hành động cần thiết
      if (!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    // Sử dụng FormData đề xử lý dữ liệu liên quẫn tới file khi gọi API
    let reqData = new FormData()
    reqData.append('avatar', e.target?.files[0])
    // Cách đề log được dữ liệu thông qua FormData
    ...
    toast.promise(
      ...
      // Lưu ý, dù có lỗi hoặc thành công thì cũng phải clear giá trị của file input, nếu không thì sẽ không thể chọn một file liên tiếp được
      e.target.value = ''
    })
  }
