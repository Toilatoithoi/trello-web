 ...
 * Hiều đơn giản: khi ứng dụng bắt đầu chạy lên, code sẽ chạy vào main.jsx đầu tiên, từ bên đó chúng ta gọi hàm injectStore ngay lập tức để gán biến mainStore vào biển axiosReduxStore cục bộ trong file này.
...
// Interceptor Respone: Can thiệp vào giữa những cái response nhận về
authorizedAxiosInstance.interceptors.response.use((response) => {
  ...
  if (error.response?.status === 410 && !originalRequests._retry) {
    // Gán thêm giá trị _retry luôn = true trong khoảng thời gian chờ, đảm bảo việc refresh token này chỉ luôn gọi 1 lần tại 1 thời điểm (nhìn lại điều kiện if ngay phía trên)
    originalRequests._retry = true

    // Kiểm tra xem nếu chưa có refreshTokenPromise thì sẽ thực hiện gán việc gọi api refresh_token đồng thời gán vào cho cái refreshTokenPromise
    ...
    return refreshTokenPromise.then(accessToken => {
      ...
      */
      // Bước 2: Bước Quan trọng: Return lại axios instance của chúng ta kết hợp các originalRequests đề gọi lại những api ban đầu bị lỗi
  ...
  // console.log error ra là sẽ thấy cấu trúc data đần tới message lỗi như dưới đây
